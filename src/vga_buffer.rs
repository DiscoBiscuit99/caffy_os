...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)] // Ensure that the struct has the exact same data layout as a `u8`.
struct ColorCode(u8);

impl ColorCode {
    fn new(foreground: Color, background: Color) -> ColorCode {
        ColorCode((background as u8) << 4 | (foreground as u8)) // First shift, then `or`.
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(C)] // Guaranttes correct field ordering.
struct ScreenChar {
    ascii_char: u8,
...
// Making the Writer global without having to carry a Writer instance around.
...
fn test_println_output() {
    ...
    interrupts::without_interrupts(|| {
        ...
        writeln!(writer, "\n{}", s).expect("writeln failed"); // Allows printing to a locked writer.
